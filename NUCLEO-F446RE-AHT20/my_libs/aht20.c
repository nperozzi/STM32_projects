...
void AHT20_init(I2C_HandleTypeDef *hi2c1)
{
	...
	HAL_Delay(40);
	uint8_t status;
	HAL_I2C_Mem_Read(hi2c1_cb, AHT20_address, AHT20_state_cmd, 1, &status, 1, HAL_MAX_DELAY);

	//Check if Calibration
	if((status >> 3 & 0x01) == 0)
	{
		AHT20_cmd_t AHT20_init_cmds[3] = {AHT20_init_cmd, AHT20_init_cmd_parameter1, AHT20_init_cmd_parameter2};
		HAL_I2C_Master_Transmit(hi2c1_cb, AHT20_address, AHT20_init_cmds, 3, HAL_MAX_DELAY);
		HAL_Delay(10);
	}
}
...
HAL_StatusTypeDef AHT20_measure(float *temperature, float *humidity)
{
	...
	if(HAL_I2C_Master_Transmit(hi2c1_cb, AHT20_address, AHT20_trigger_measurement_cmds, 3, HAL_MAX_DELAY) != HAL_OK)
	{
		return HAL_ERROR;
	}
	HAL_Delay(80);
	...
	do
	{
		if(HAL_I2C_Mem_Read(hi2c1_cb, AHT20_address, AHT20_state_cmd, 1, &status, 1, HAL_MAX_DELAY) != HAL_OK)
		{
			return HAL_ERROR;
		}
		HAL_Delay(10);
	...
	if(HAL_I2C_Master_Receive(hi2c1_cb, AHT20_address, received_data, 7, HAL_MAX_DELAY) != HAL_OK)
	{
		return HAL_ERROR;
	}
